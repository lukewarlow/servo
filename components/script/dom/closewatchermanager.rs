...
pub struct CloseWatcherManager {
    groups: DomRefCell<Vec<Vec<DomRoot<CloseWatcher>>>>,
...
impl CloseWatcherManager {
    pub fn new(
        window: &Window,
    ) -> DomRoot<CloseWatcherManager> {
        let close_watcher_manager = reflect_dom_object(
            Box::new(CloseWatcherManager::new_inherited()),
            window,
        );

        close_watcher_manager
    }
    pub fn new_inherited() -> CloseWatcherManager {
        CloseWatcherManager {
            groups: DomRefCell::new(Vec::new()),
            allowed_number_of_groups: 1,
            next_user_interaction_allows_a_new_group: true
        }
    }

    pub fn add(&self, close_watcher: &DomRoot<CloseWatcher>) {
    ...
    pub fn remove(&self, close_watcher: &CloseWatcher) {
    ...
    pub fn process_close_watchers(&mut self) -> bool {
        ...
        if !self.groups.borrow().is_empty() {
            ...
            for close_watcher in group.iter().rev() {
                ...
                let should_proceed = close_watcher.request_to_close();
    ...
    pub fn can_prevent_close(&self) -> bool {
        ...
        return self.groups.borrow().len() < self.allowed_number_of_groups;
    }
}
